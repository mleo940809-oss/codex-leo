...
| V2.21 | 2026-05-13 | 收口活动新增、编辑、修改页的基础信息区字段与展示口径，统一使用模式与券码分配模式命名，并补齐新增页基础信息只读区 | Codex | 待评审签核 |

## 需求列表

| 业务需求 / Epic | 产品需求 / User Story | 产品需求描述 | 记录日期 | 产品发布版本 | 提出部门 |
|---|---|---|---|---|---|
| Weixin摇一摇活动管理 | 统一创建、维护、查看和执行商品券活动 | 覆盖活动列表、活动新增、活动编辑、活动修改、活动详情与批次管理，统一活动创建、草稿接续、生效后展示修正、只读详情与批次发放管理能力 | 2026-05-11 | V2.21 | 业务方待确认 |
...
| 图文件 | 可编辑源文件 | 图示说明 |
|---|---|---|
| "E:\Codex\New_rules\docs\versions\Weixin摇一摇\Weixin摇一摇_功能架构图_v1.4_20260510.jpg" | 根节点为 Weixin摇一摇；一级分支为活动管理与批次管理；活动管理覆盖活动列表/新增/编辑/修改/详情，批次管理覆盖批次列表/预配置/券码结果自动生成与回传/门店范围/发放控制/批次状态管理 / 发放状态管理 |
...
| 图文件 | 可编辑源文件 | 图示说明 |
|---|---|---|
| "E:\Codex\New_rules\docs\versions\Weixin摇一摇\Weixin摇一摇_整体流程图_v1.5_20260511.jpg" | 活动列表页、活动新增/编辑/修改/详情、批次管理页、批次预配置/券码结果自动生成与回传/门店关联/发放控制与状态结果的主流程，含草稿恢复、校验失败和失效处理 |
...
| 图文件 | 可编辑源文件 | 图示说明 |
|---|---|---|
| "E:\Codex\New_rules\docs\versions\Weixin摇一摇\Weixin摇一摇_活动列表页流程图_v1.0_20260511.jpg"| 活动列表页主流程、状态分支、失效二次确认与返回刷新规则 |
...
- 活动基础属性 `单券模式` 与 `系统自动生成 / 回传` 为系统固定值，不作为页面可配置前置条件。
...
15. `单券模式` 与 `系统自动生成 / 回传` 为系统固定属性，不作为列表筛选条件，也不作为可编辑字段展示。
...
- `单券模式` 与 `系统自动生成 / 回传` 均为系统固定属性，不作为列表筛选条件，也不在列表中作为可编辑字段展示。
...
3. 进入创建表单页后，先展示基础信息区，再展示当前券型相关字段，不展示另一种彩票型内容。
4. 创建表单页按 `基础信息区`、`基础配置` 和 `更多配置` 组织内容，其中 `更多配置` 默认收起。
...
| 图文件 | 可编辑源文件 | 图示说明 |
|---|---|---|
| [Weixin摇一摇_活动新增页角色用例图_v1.0_20260511.png](E:/Codex/New_rules/docs/versions/Weixin摇一摇/Weixin摇一摇_活动新增页角色用例图_v1.0_20260511.png) | [Weixin摇一摇_活动新增页角色用例图_v1.0_20260511.drawio](E:/Codex/New_rules/docs/versions/Weixin摇一摇/Weixin摇一摇_活动新增页角色用例图_v1.0_20260511.drawio) | 运营人员围绕活动新增页完成券类型选择、表单创建、草稿保存、提交审核与批次门禁校验的角色边界 |

###### 2.1.2.2.2 产品流程图

| 图文件 | 可编辑源文件 | 图示说明 |
|---|---|---|
| "E:\Codex\New_rules\docs\versions\Weixin摇一摇\Weixin摇一摇_活动新增页流程图_v1.0_20260511.jpg"| 券类型选择、创建表单填写、草稿保存、提交审核、批次门禁校验与返回详情的主流程 |
...
| 创建表单页 | 见 [Weixin摇一摇_活动新增页_v1.8_20260510.md](E:/Codex/New_rules/docs/versions/Weixin摇一摇/Weixin摇一摇_活动新增页_v1.8_20260510.md) | [Weixin摇一摇_活动新增页_v1.8_20260510.md](E:/Codex/New_rules/docs/versions/Weixin摇一摇/Weixin摇一摇_活动新增页_v1.8_20260510.md) | 基础信息区 + 基础配置 + 差异区 + 提交确认，支持草稿保存与提交审核 |

###### 2.1.2.6.1 功能逻辑说明

1. 券类型选择页仅提供 `满减券`、`兑换券` 两个入口和 `取消` 动作。
2. 用户选择券类型后进入对应创建表单页，并锁定当前券类型，不允许中途切换。
3. 创建表单页由 `基础信息区`、`基础配置` 和 `更多配置` 组成，其中 `更多配置` 默认收起。
4. `基础信息区` 默认展示 `商品券ID`、`商品券类型`、`外部商品ID`、`使用模式`、`券码分配模式`；除 `外部商品ID` 外其余字段均为只读。
5. 基础配置默认展示当前券型相关的主字段和核心字段，另一个券型的字段不展示。
6. 公共主字段默认展示 `商品券名称`、`商品券图片`、`商品券背景图`、`商品券标题文案`、`规则摘要`；`商品券详情图`、`商品券副标题` 归入 `更多配置`。
7. `使用模式` 固定为 `单券模式`，`券码分配模式` 固定为 `系统自动生成 / 回传`，两者仅展示不可修改。
8. 满减券默认展示 `门槛金额`、`减免金额`，`满减说明` 归入 `更多配置`。
9. 兑换券默认展示 `商品原价`、`适用商品组合`，`兑换说明` 归入 `更多配置`。
10. `适用商品组合` 默认展开，且每个组合至少包含 `组合名称`、`可选单品数量` 和 `单品列表`。
11. `单品列表` 默认给 1 条空白单品，支持继续新增与删除，但不允许删到 0 条；如需清空商品组，直接删除整个商品组并重新创建。
12. 草稿保存允许主字段未完整填写，但必须满足最小结构约束；至少需具备 `商品券名称`、`外部商品ID`、`券类型`、`使用模式` 这四个最小识别信息，兑换券草稿还至少需包含 1 个商品组。
13. 提交审核前必须满足字段格式、字段联动、组合校验以及批次门禁要求；若未满足至少 1 个已完成预配置且通过校验的批次，`提交审核` 按钮置灰不可点。
14. 提交成功后返回活动详情页，并将活动状态置为 `审批中`。
15. 若用户返回券类型选择页或列表页，且存在未保存更改，系统应先二次确认。
...
| 4 | 商品券ID | 查看 | 只读展示活动唯一标识，提交后自动生成 |
| 5 | 商品券类型 | 查看 | 只读展示当前券类型 |
| 6 | 外部商品ID | 输入 / 查看 | 活动未产生批次时可编辑，产生批次后全链路只读 |
| 7 | 使用模式 | 查看 | 只读展示固定值 `单券模式` |
| 8 | 券码分配模式 | 查看 | 只读展示固定值 `系统自动生成 / 回传` |
| 9 | 商品券名称 | 输入 | 填写页面展示标题，需满足长度与字符限制 |
| 10 | 商品券图片 | 上传 | 上传商品券展示图片，需满足尺寸和格式要求 |
| 11 | 商品券背景图 | 上传 | 上传商品券背景图片，需满足尺寸和格式要求 |
| 12 | 商品券详情图 | 上传 | 最多上传 8 张详情图，超出上限阻止继续添加 |
| 13 | 商品券标题文案 | 输入 | 填写页面展示标题，需满足长度与字符限制 |
| 14 | 商品券副标题 | 输入 | 填写辅助展示文案，支持选填 |
| 15 | 规则摘要 | 输入 | 填写前台可读规则概述，需满足长度与字符限制 |
| 16 | 更多配置 | 展开 / 收起 | 展示当前券型的低频扩展字段，默认收起 |
| 17 | 门槛金额 | 输入 | 满减券中填写触发门槛，支持两位小数 |
| 18 | 减免金额 | 输入 | 满减券中填写减免金额，支持两位小数并受联动限制 |
| 19 | 商品原价 | 输入 | 兑换券中填写原价，支持两位小数并受范围限制 |
| 20 | 适用商品组合 | 增删改 | 配置兑换券商品组合，至少 1 个，最多 50 个 |
| 21 | 单品列表 | 增删改 | 配置组合内单品，至少 1 个，最多 99 个 |
| 22 | 保存草稿 | 点击 | 保存当前内容为草稿，允许主字段未完整填写，但需满足最小结构约束 |
| 23 | 提交审核 | 点击 | 校验通过且满足批次门禁后创建活动并进入审批中 |
| 24 | 返回类型选择 | 点击 | 返回券类型选择页，未保存内容不保留 |
| 25 | 返回列表页 | 点击 | 放弃当前创建并返回列表页 |

###### 2.1.2.6.3 字段说明

下列表格仅描述字段全集，默认展示层级按前述业务规则执行：`基础信息区` 默认展示，`基础配置` 和 `更多配置` 中的低频项按规则折叠。

基础信息区：

| 序号 | 字段 | 数据类型 | 数据限制 | 是否必填 | 描述 | 默认值 | 数据来源 | 数据是否可编辑 | 数据字典 | 数据字典说明 |
|---:|---|---|---|---|---|---|---|---|---|---|
| 1 | 商品券ID | 文本 | 提交后自动生成 | 否 | 当前活动唯一标识 | 提交后生成 | 系统生成 | 不可编辑 | 无 | 提交后只读 |
| 2 | 商品券类型 | 枚举 | 固定值 `满减券` / `兑换券` | 是 | 当前券类型 | 无 | 用户选择 | 不可编辑 | 券类型字典 | 选择后锁定 |
| 3 | 外部商品ID | 文本 | 必填；提交前不能为空；活动未产生批次时可编辑，产生批次后全链路只读 | 是 | 外部商品系统的商品唯一标识 | 无 | 用户输入 | 条件可编辑 | 无 | 未产生批次时可编辑 |
| 4 | 使用模式 | 枚举 | 固定值 `单券模式` | 是 | 使用模式固定展示 | `单券模式` | 系统生成 | 不可编辑 | 使用模式字典 | 仅展示单券模式 |
| 5 | 券码分配模式 | 枚举 | 固定值 `系统自动生成 / 回传` | 是 | 券码分配方式 | `系统自动生成 / 回传` | 系统生成 | 不可编辑 | 券码分配模式字典 | 只读展示 |
...
- 表单默认分为 `基础信息区`、`基础配置` 与 `更多配置`，其中 `更多配置` 默认收起。
- `基础信息区` 默认展示 `商品券ID`、`商品券类型`、`外部商品ID`、`使用模式`、`券码分配模式`，其中除 `外部商品ID` 外其余字段均为只读。
- `商品券名称`、`商品券图片`、`商品券背景图`、`商品券标题文案`、`规则摘要` 为公共主字段。
- `商品券详情图`、`商品券副标题`、`满减说明`、`兑换说明`、`单品图片`、`小程序 AppID`、`小程序路径` 均属于扩展项，默认收起。
- `使用模式` 为系统固定值 `单券模式`，`券码分配模式` 为系统固定值 `系统自动生成 / 回传`，均仅展示不可修改。
...
- 保存草稿允许主字段未完整填写，但必须满足最小结构约束；至少需具备 `商品券名称`、`外部商品ID`、`券类型`、`使用模式` 这四个最小识别信息，兑换券草稿还至少需包含 1 个商品组。
...
| 图文件 | 可编辑源文件 | 图示说明 |
|---|---|---|
| [Weixin摇一摇_活动编辑页角色用例图_v1.0_20260511.png](E:/Codex/New_rules/docs/versions/Weixin摇一摇/Weixin摇一摇_活动编辑页角色用例图_v1.0_20260511.png) | [Weixin摇一摇_活动编辑页角色用例图_v1.0_20260511.drawio](E:/Codex/New_rules/docs/versions/Weixin摇一摇/Weixin摇一摇_活动编辑页角色用例图_v1.0_20260511.drawio) | 运营人员在草稿补全、保存草稿、提交审核与状态阻断场景中的角色边界 |

###### 2.1.3.2.2 产品流程图

| 图文件 | 可编辑源文件 | 图示说明 |
|---|---|---|
| "E:\Codex\New_rules\docs\versions\Weixin摇一摇\Weixin摇一摇_活动编辑页流程图_v1.0_20260511.jpg" | 草稿补全、保存草稿、提交审核、取消与状态阻断的主流程 |
...
| 页面名称 | 图文件 | 可编辑源文件 | 图示说明 |
|---|---|---|---|
| 活动编辑页 | 见 [Weixin摇一摇_活动编辑页_v1.8_20260510.md](E:/Codex/New_rules/docs/versions/Weixin摇一摇/Weixin摇一摇_活动编辑页_v1.8_20260510.md) | [Weixin摇一摇_活动编辑页_v1.8_20260510.md](E:/Codex/New_rules/docs/versions/Weixin摇一摇/Weixin摇一摇_活动编辑页_v1.8_20260510.md) | 草稿补全、保存草稿、提交审核、取消 |

###### 2.1.3.6.1 功能逻辑说明

1. 页面基础信息区展示 `商品券ID`、`商品券类型`、`外部商品ID`、`使用模式`、`券码分配模式`，其中除 `外部商品ID` 外其余字段均为只读。
...
5. `使用模式` 为系统固定值 `单券模式`，`券码分配模式` 为系统固定值 `系统自动生成 / 回传`，两者均不允许修改。
6. 满减券差异区默认展示 `门槛金额`、`减免金额`，`满减说明` 进入 `更多配置`。
7. 兑换券差异区默认展示 `商品原价` 和 `适用商品组合`，`兑换说明` 进入 `更多配置`。
8. 兑换券的 `适用商品组合` 默认展开；组合内至少包含 `组合名称`、`可选单品数量` 和 `单品列表`。
9. 单品列表至少包含 `单品名称`、`单品价格`、`单品数量`，并可选配置 `单品图片`、`小程序 AppID`、`小程序路径`；这些低频扩展项在无数据时折叠，有数据时自动展开。
10. 当前券型的 `更多配置` 仅展示本券型对应内容，不展示另一种彩票型配置。
11. 保存草稿前必须校验字段格式、字段联动关系和当前活动状态；提交审核前还需校验批次门禁。
12. 页面不承接 `Code` 上传、门店关联和批次执行相关能力。
13. 当活动状态进入 `EFFECTIVE` 或 `DEACTIVATED` 时，本页不再可编辑；当活动下已产生 1 个及以上批次时，`外部商品ID` 也必须保持只读。
...
| 5 | 券码分配模式 | 查看 | 只读展示固定值 `系统自动生成 / 回传` |
| 6 | 商品券名称 | 输入 | 支持中文、英文、数字、空格及常用符号，首尾空格自动去除 |
| 7 | 商品券图片 | 上传 | 上传商品券展示图片，需满足尺寸和格式要求 |
| 8 | 商品券背景图 | 上传 | 上传商品券背景图片，需满足尺寸和格式要求 |
| 9 | 商品券详情图 | 上传 | 低频项；无数据时折叠，有数据时自动展开 |
| 10 | 商品券标题文案 | 输入 | 填写页面展示标题，需满足长度与字符限制 |
| 11 | 商品券副标题 | 输入 | 低频项；无数据时折叠，有数据时自动展开 |
| 12 | 规则摘要 | 输入 | 填写前台可读规则概述，需满足长度与字符限制 |
| 13 | 门槛金额 | 输入 | 满减券中填写触发门槛，支持两位小数 |
| 14 | 减免金额 | 输入 | 满减券中填写减免金额，支持两位小数并受联动限制 |
| 15 | 满减说明 | 输入 | 低频项；无数据时折叠，有数据时自动展开 |
| 16 | 商品原价 | 输入 | 兑换券中填写原价，支持两位小数并受范围限制 |
| 17 | 适用商品组合 | 增删改 | 配置兑换券商品组合，至少 1 个，最多 50 个 |
| 18 | 单品列表 | 增删改 | 配置组合内单品，至少 1 个，最多 99 个 |
| 19 | 兑换说明 | 输入 | 低频项；无数据时折叠，有数据时自动展开 |
| 20 | 保存草稿 | 点击 | 保存当前编辑内容为草稿 |
| 21 | 提交审核 | 点击 | 校验通过且满足批次门禁后进入审批中 |
| 22 | 取消 | 点击 | 放弃当前修改，存在未保存变更时需二次确认 |
...
| 4 | 使用模式 | 枚举 | 固定值 `单券模式` | 否 | 当前使用模式 | `单券模式` | 系统生成 | 不可编辑 | 使用模式字典 | 只读展示 |
| 5 | 券码分配模式 | 枚举 | 固定值 `系统自动生成 / 回传` | 否 | 当前券码分配模式 | `系统自动生成 / 回传` | 系统生成 | 不可编辑 | 券码分配模式字典 | 只读展示 |
...
- 编辑页只允许 `草稿` 活动进入。
- `商品券ID`、`商品券类型`、`外部商品ID`、`使用模式`、`券码分配模式` 均为基础信息；其中 `商品券ID`、`商品券类型`、`使用模式`、`券码分配模式` 为只读，`外部商品ID` 在活动未产生批次时可编辑，活动产生 1 个及以上批次后在任何页面均只读。
...
| 图文件 | 可编辑源文件 | 图示说明 |
|---|---|---|
| [Weixin摇一摇_活动修改页角色用例图_v1.0_20260511.png](E:/Codex/New_rules/docs/versions/Weixin摇一摇/Weixin摇一摇_活动修改页角色用例图_v1.0_20260511.png) | [Weixin摇一摇_活动修改页角色用例图_v1.0_20260511.drawio](E:/Codex/New_rules/docs/versions/Weixin摇一摇/Weixin摇一摇_活动修改页角色用例图_v1.0_20260511.drawio) | 运营人员与审核/管理角色在生效中展示修正、只读快照与状态阻断场景中的角色边界 |

###### 2.1.4.2.2 产品流程图

| 图文件 | 可编辑源文件 | 图示说明 |
|---|---|---|
| "E:\Codex\New_rules\docs\versions\Weixin摇一摇\Weixin摇一摇_活动修改页流程图_v1.0_20260511.jpg" | 生效中展示修正、保存修改、回流详情页刷新快照、取消与状态阻断的主流程 |
...
| 页面名称 | 图文件 | 可编辑源文件 | 图示说明 |
|---|---|---|---|
| 活动修改页 | 见 [Weixin摇一摇_活动修改页_v1.3_20260510.md](E:/Codex/New_rules/docs/versions/Weixin摇一摇/Weixin摇一摇_活动修改页_v1.3_20260510.md) | [Weixin摇一摇_活动修改页_v1.3_20260510.md](E:/Codex/New_rules/docs/versions/Weixin摇一摇/Weixin摇一摇_活动修改页_v1.3_20260510.md) | 生效中展示修正、只读快照、保存后回流详情页 |

###### 2.1.4.6.1 功能逻辑说明

1. 页面基础信息只读区展示 `商品券ID`、`商品券类型`、`外部商品ID`、`使用模式`、`券码分配模式`，均为只读固定值。
2. `外部商品ID` 在本页展示为只读快照，不提供编辑入口。
3. 高风险配置只读区展示当前券规则快照，满减券包括 `门槛金额`、`减免金额`、`满减说明`，兑换券包括 `商品原价`、`适用商品组合`、`组合名称`、`可选单品数量`、`单品列表`、`单品名称`、`单品价格`、`单品数量`、`小程序 AppID`、`小程序路径`。
...
| 图文件 | 可编辑源文件 | 图示说明 |
|---|---|---|
| "E:\Codex\New_rules\docs\versions\Weixin摇一摇\Weixin摇一摇_活动详情页流程图_v1.0_20260512.jpg"| 详情页只读快照、批次概览、状态核对、编辑/修改/批次管理入口与失效确认的主流程 |
...
| 6 | 券码分配模式 | 枚举 | 只读展示固定值 `系统自动生成 / 回传` | 否 | 系统固定值 | 系统自动生成 / 回传 | 系统快照 | 不可编辑 | 券码分配模式字典 | 无 |
...
| 图文件 | 可编辑源文件 | 图示说明 |
|---|---|---|
| [Weixin摇一摇_批次管理页角色用例图_v1.0_20260511.png](E:/Codex/New_rules/docs/versions/Weixin摇一摇/Weixin摇一摇_批次管理页角色用例图_v1.0_20260511.png) | [Weixin摇一摇_批次管理页角色用例图_v1.0_20260511.drawio](E:/Codex/New_rules/docs/versions/Weixin摇一摇/Weixin摇一摇_批次管理页角色用例图_v1.0_20260511.drawio) | 围绕批次管理页的活动上下文查看、批次预配置、保存校验、发放控制、失效确认与只读查看的角色边界 |

###### 2.1.6.2.2 产品流程图

| 图文件 | 可编辑源文件 | 图示说明 |
|---|---|---|
| "E:\Codex\New_rules\docs\versions\Weixin摇一摇\Weixin摇一摇_批次管理页流程图_v1.0_20260512.jpg"| 批次管理页主流程、批次预配置、保存并校验、系统自动生成 / 回传、过程态待提交审核与失效二次确认的主流程 |
...
| 页面名称 | 图文件 | 可编辑源文件 | 图示说明 |
|---|---|---|---|
| 批次管理页 | 无独立冻结原型 | 无 | 以本章节结构为准，后续需同步原型 |
...
|---|---|---|---|---|---|---|---|
| 活动 | 活动 | 活动ID、名称、外部商品ID、类型、状态、使用模式、券码分配模式 | 活动层主对象 | 对象 | 是 | 系统/用户输入 | 活动层状态主源 |
...
| TC-010 | EDIT-02 | 规则流程 | 使用模式和券码分配模式保持只读 | 进入活动编辑页 | 检查并尝试修改固定字段 | 草稿活动 | 使用模式和券码分配模式不可改动 |
