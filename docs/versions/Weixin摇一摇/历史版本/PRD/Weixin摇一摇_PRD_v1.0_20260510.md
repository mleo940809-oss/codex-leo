...
|---|---|---|---|---|---|
| Weixin摇一摇活动管理 | 统一创建、维护、查看和执行商品券活动 | 覆盖活动列表、活动新增、活动编辑、活动修改、活动详情与批次管理，统一活动创建、草稿接续、生效前修订、只读详情与批次发放管理能力 | 2026-05-10 | V1.0 | 业务方待评审确认 |
...
- 活动列表支持按状态与时间窗口检索，并以部分重叠规则筛选活动使用时间范围
...
5. `活动使用时间范围` 仅用于查找与查询时间范围存在**部分重叠**的活动，不要求完全覆盖。
...
| 活动使用时间范围 | 当前活动下批次的日期窗口 | 是 | 仅展示汇总窗口 |
...
|---|---|---|---|---|---|
| 活动列表检索 | 活动列表页（待绑定） | 列表态 | 搜索、筛选、跳转 | 时间范围部分重叠 | 否 |
| 活动新增 | 活动新增页（待绑定） | 草稿/审核中 | 选券类型、填写、提交 | 草稿存在直接跳转编辑页 | 否 |
| 活动编辑 | 活动编辑页（待绑定） | 草稿/审核中 | 继续补全、保存、提交 | 生效后不可编辑 | 否 |
| 活动修改 | 活动修改页（待绑定） | 生效中 | 低风险字段修订 | 高风险字段只读 | 否 |
| 活动详情 | 活动详情页（待绑定） | 全部只读 | 查看、跳转 | 批次动作不在本页展开 | 否 |
| 批次管理 | 批次管理页（待绑定） | 审核中/发放中/暂停/失效 | 预配置、上传、发放、暂停 | Code 校验、门店关联、预预算分离 | 否 |
...
| 结论 | 说明 | 评审人 | 日期 |
|---|---|---|---|
| 可上评审（冻结待签） | 当前文档已完成正式模板起草，并完成第 4 至第 9 章冻结；第 10 章用于留存正式签核与归档记录 | 评审会确认 | 2026-05-10 |

### 10.2 确认签核

> 以下签核栏位为正式冻结留痕位，当前先保留签核骨架，后续由业务、产品、技术、测试分别补齐确认人、签字与日期。
...
> 正式原型必须是可点击、可验证、可回溯的交互原型；仅有截图或静态导出不视为通过。

| 验收项 | 通过条件 | 验收结果 | 验收人 | 日期 |
|---|---|---|---|---|
| 原型活链接 | 必须可打开、可点击、可预览 | 待原型补齐 | 待签核 | 待签核 |
| 主路径可走 | 核心主流程可逐步完成 | 待原型补齐 | 待签核 | 待签核 |
| 分支与异常 | 分支流程、空态、加载态、错误态可验证 | 待原型补齐 | 待签核 | 待签核 |
| 返回与收尾 | 返回、关闭、撤销、确认等收尾动作可用 | 待原型补齐 | 待签核 | 待签核 |
| 权限与边界 | 权限差异、不可编辑态、越权提示可验证 | 待原型补齐 | 待签核 | 待签核 |

### 10.4 需求交付包输出

| 输出件 | 是否必交付 | 内容要求 | 关联位置 / 链接 |
|---|---|---|---|
| PRD 正文 | 是 | 与冻结版本一致，结构完整 | 本文件 |
| 交付包清单 | 是 | 列出所有交付物及文件名 | 见本章与第 10.5、10.6 章 |
| 冻结/归档记录 | 是 | 记录冻结版本、时间、责任人 | `docs/prototype/Weixin摇一摇/archive/Weixin摇一摇_v1.0_20260510/archive-manifest.md` |
| 验收标准 | 是 | 明确通过/不通过条件 | 本文件 |
| 附件清单 | 是 | 原型、流程图、字段表、规则表等 | 见本文件第 4-9 章及原型绑定记录 |
| 原型活链接 | 是 | 必须可点击、可交互 | `docs/prototype/Weixin摇一摇/binding.md` |
| 一致性矩阵 | 是 | 功能点逐项映射到原型 | 本文件 5.7 |
| 原型交互验收记录 | 是 | 记录主路径、分支、异常、权限验收结果 | `docs/prototype/Weixin摇一摇/binding.md` |
| 依赖说明 | 是 | 前置条件、外部输入和未决事项 | 本文件 6.3 |
| 交付确认记录 | 是 | 签核、确认人和确认日期 | `docs/prototype/Weixin摇一摇/archive/Weixin摇一摇_v1.0_20260510/archive-manifest.md` |

### 10.5 原型绑定记录

> 正式原型绑定记录固定路径应为 `docs/prototype/<文档名>/binding.md`。当前文档已落地正式绑定记录骨架，冻结时在该路径补齐最终链接与签核信息。

| 绑定项 | 要求 | 说明 | 记录位置 |
|---|---|---|---|
| 原型活链接 | 是 | 可打开、可点击、可预览的正式原型链接 | `docs/prototype/Weixin摇一摇/binding.md` |
| 对应 PRD 版本 | 是 | 记录关联的 PRD 冻结版本号 | `docs/prototype/Weixin摇一摇/binding.md` |
| 原型版本 / 页面范围 | 是 | 记录原型名称、页面范围、版本号或设计稿标识 | `docs/prototype/Weixin摇一摇/binding.md` |
| 源文件路径 | 是 | 记录 Figma / HTML / 其他可编辑源文件链接 | `docs/prototype/Weixin摇一摇/binding.md` |
| 确认人 / 确认时间 | 是 | 记录冻结确认责任人与时间 | `docs/prototype/Weixin摇一摇/binding.md` |
| 留档结果 | 是 | 记录是否已归档到正式留档路径 | `docs/prototype/Weixin摇一摇/binding.md` |

### 10.6 原型归档记录

> 原型冻结后应形成归档目录，例如 `docs/prototype/Weixin摇一摇/archive/Weixin摇一摇_v<版本号>_<YYYYMMDD>/archive-manifest.md`。当前文档已落地正式归档目录骨架，冻结时在该路径补齐最终归档内容。

| 归档项 | 要求 | 说明 | 记录位置 |
|---|---|---|---|
| 归档清单 | 是 | 记录本次冻结归档的内容 | `docs/prototype/Weixin摇一摇/archive/Weixin摇一摇_v1.0_20260510/archive-manifest.md` |
| 原型活链接 | 是 | 记录冻结时可回读的正式原型链接 | `docs/prototype/Weixin摇一摇/archive/Weixin摇一摇_v1.0_20260510/archive-manifest.md` |
| 对应 PRD 版本 | 是 | 记录关联的 PRD 冻结版本号 | `docs/prototype/Weixin摇一摇/archive/Weixin摇一摇_v1.0_20260510/archive-manifest.md` |
| 源文件 / 导出文件 | 是 | 记录 Figma / HTML / 导出包位置 | `docs/prototype/Weixin摇一摇/archive/Weixin摇一摇_v1.0_20260510/archive-manifest.md` |
| 关键页面预览图 | 是 | 记录冻结时的页面预览证据 | `docs/prototype/Weixin摇一摇/archive/Weixin摇一摇_v1.0_20260510/archive-manifest.md` |
| 归档确认人 / 时间 | 是 | 记录归档责任人和时间 | `docs/prototype/Weixin摇一摇/archive/Weixin摇一摇_v1.0_20260510/archive-manifest.md` |
...
以下事项为当前 PRD 上评审时需要确认的外部输入，不影响本稿进入评审，但应在评审会上逐项确认：

| 待决项 | 当前建议 | 评审确认方式 |
|---|---|---|
| 业务负责人 | 由业务方在评审会上指定 | 现场签核 |
| 产品负责人 | 由产品侧在评审会上指定 | 现场签核 |
| 活动服务接口 | 按第 6.3 节约定对齐 | 接口评审确认 |
| 批次服务接口 | 按第 6.3 节约定对齐 | 接口评审确认 |
| Code 文件处理 | 按第 6.3 节约定对齐 | 接口评审确认 |
| 门店服务 | 按第 6.3 节约定对齐 | 接口评审确认 |
| 通知服务 | 按第 6.3 节约定对齐 | 接口评审确认 |
| 列表查询性能基线 | 评审后由研发与测试补充 | 评审后补录 |
| 详情加载性能基线 | 评审后由研发与测试补充 | 评审后补录 |
| 浏览器兼容范围 | 评审后由前端与测试补充 | 评审后补录 |
| 交付包清单的最终附件 | 评审后随冻结包补齐 | 冻结前补录 |
...
- 若字段暂未最终确认，优先写明确的评审待决项，不要留成散装空白
