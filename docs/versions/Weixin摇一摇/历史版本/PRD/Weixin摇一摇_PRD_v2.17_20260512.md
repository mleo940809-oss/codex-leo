...
|---|---|---|---|---|---|
| Weixin摇一摇活动管理 | 统一创建、维护、查看和执行商品券活动 | 覆盖活动列表、活动新增、活动编辑、活动修改、活动详情与批次管理，统一活动创建、草稿接续、生效后展示修正、只读详情与批次发放管理能力 | 2026-05-11 | V2.17 | 业务方待确认 |
...
2. 基础信息区展示 `商品券名称`、`商品券ID`、`商品券类型`、`外部商品ID`、`使用模式`、`券Code分配模式`、`状态`。
...
6. `编辑`、`修改`、`批次管理`、`失效` 等按钮必须随活动状态控制展示和可点击性；其中 `审批中` 仅保留查看入口，不显示编辑、修改和批次管理入口。
...
| 9 | 状态标签 | 查看 | 识别当前可执行入口 |
| 10 | 商品券主图 / 背景图 / 详情图 | 查看 | 预览活动视觉信息 |
| 11 | 批次概览 | 点击 | 跳转到批次管理页或对应批次视图 |
...
| 4 | 外部商品ID | 文本 | 只读展示 | 是 | 外部商品系统的商品唯一标识 | 无 | 系统快照 | 不可编辑 | 无 | 无 |
| 5 | 使用模式 | 枚举 | 只读展示固定值 `SINGLE` | 否 | 系统固定值 | `SINGLE` | 系统快照 | 不可编辑 | 使用模式字典 | 无 |
| 6 | 券Code分配模式 | 枚举 | 只读展示固定值 `UPLOAD` | 否 | 系统固定值 | `UPLOAD` | 系统快照 | 不可编辑 | 分配模式字典 | 无 |
...
2. 活动摘要区仅展示当前活动的上下文信息，不承载编辑入口，且包含 `外部商品ID` 的只读快照展示。
...
| TC-016 | BATCH-01 | 正常流程 | 批次页预配置并保存基础信息 | 活动为草稿或生效中 | 新建批次并配置日期窗、预算与基础信息 | 批次配置数据 | 配置成功并保存 |
| TC-017 | BATCH-02 | 规则流程 | 批次日期窗、预算与门店范围联动校验 | 批次为可配置状态 | 配置日期窗、预算和门店范围 | 批次配置数据 | 不合法配置被拦截并提示原因 |
| TC-018 | BATCH-03 | 校验流程 | Code 文件上传与合法性校验 | 批次为可上传状态 | 上传 Code 文件并保存校验 | 非法/合法文件 | 合法文件通过，非法文件阻止保存 |
| TC-019 | BATCH-04 | 正常流程 | 门店关联保存并回显 | 门店范围允许关联门店 | 选择门店并保存关联结果 | 门店列表 | 保存成功并回显关联门店数 |
| TC-020 | BATCH-05 | 正常流程 | 通知配置保存与回显 | 批次通知配置区域可编辑 | 修改通知配置并保存 | 通知配置数据 | 保存成功且页面回显最新配置 |
| TC-021 | BATCH-06 | 状态控制 | 发放、暂停、停止与失效动作受状态限制 | 批次处于发放中、已暂停或已停止状态 | 尝试执行受限动作 | 受限状态批次 | 按状态限制操作，不允许越权动作 |
...
| 批次管理 | [Weixin摇一摇_批次管理页_v1.7_20260510.md](E:/Codex/New_rules/docs/versions/Weixin摇一摇/Weixin摇一摇_批次管理页_v1.7_20260510.md) | 审批中/发放中/暂停/失效 | 预配置、上传、发放、暂停 | Code 校验、门店关联、预预算分离 | 是 |
