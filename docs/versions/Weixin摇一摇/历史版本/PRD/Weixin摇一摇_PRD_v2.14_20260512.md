...
|---|---|---|---|---|---|
| Weixin摇一摇活动管理 | 统一创建、维护、查看和执行商品券活动 | 覆盖活动列表、活动新增、活动编辑、活动修改、活动详情与批次管理，统一活动创建、草稿接续、生效前修订、只读详情与批次发放管理能力 | 2026-05-11 | V2.14 | 业务方待确认 |
...
活动编辑页用于草稿补全，是活动层唯一的继续编辑入口。该页承接首次创建后的内容补全，但不承接审批中的修订，不承接生效后的修改，不承接 `Code` 配置，不承接批次执行能力。
...
3. `商品券名称`、`商品券图片`、`商品券背景图`、`商品券详情图`、`商品券标题文案`、`商品券副标题`、`规则摘要`、`使用模式` 按设计要求进行展示，其中 `使用模式` 为只读固定值 `SINGLE`。
4. 公共字段区支持对活动基础展示信息进行补全或修订。
5. 满减券差异区展示 `门槛金额`、`减免金额`、`满减说明`，用于活动层生效前修订。
6. 兑换券差异区展示 `商品原价`、`适用商品组合`、`兑换说明`，用于活动层生效前修订。
7. 兑换券的 `适用商品组合` 为可重复配置区，组合内至少包含 `组合名称`、`可选单品数量` 和 `单品列表`。
8. 单品列表至少包含 `单品名称`、`单品价格`、`单品数量`，并可选配置 `单品图片`、`小程序 AppID`、`小程序路径`。
9. 保存修改前必须校验字段格式、字段联动关系和当前活动状态。
10. 页面不承接 `Code` 上传、门店关联和批次执行相关能力。
11. 当活动状态进入 `EFFECTIVE` 或 `DEACTIVATED` 时，本页不再可编辑；当活动下已产生 1 个及以上批次时，`外部商品ID` 也必须保持只读。
...
| 14 | 满减说明 | 输入 | 填写满减券补充说明，支持选填 |
| 15 | 商品原价 | 输入 | 兑换券中填写原价，支持两位小数并受范围限制 |
| 16 | 适用商品组合 | 增删改 | 配置兑换券商品组合，至少 1 个，最多 50 个 |
| 17 | 单品列表 | 增删改 | 配置组合内单品，至少 1 个，最多 99 个 |
| 18 | 兑换说明 | 输入 | 填写兑换券补充说明，支持选填 |
| 19 | 保存修改 | 点击 | 保存当前编辑内容，保留在生效前流程中 |
| 20 | 取消 | 点击 | 放弃当前修改，存在未保存变更时需二次确认 |
...
- 保存修改前必须完成字段格式、字段联动和状态校验；`外部商品ID` 仅在活动未产生批次时允许修改。
...
4. 低风险可编辑区仅开放 `商品券图片`、`商品券背景图`、`商品券详情图`、`商品券标题文案`、`商品券副标题`、`规则摘要`。
5. 低风险字段修改只影响后续新发券，不影响已发券和历史记录。
6. 页面不承接 `Code` 上传、门店关联、预算配置和发放控制。
7. 保存修改后必须回流活动详情页，并刷新本地快照，避免用户看到旧数据。
8. 当活动状态为 `AUDITING` 或 `DEACTIVATED` 时，不允许继续编辑。
...
| 4 | 使用模式 | 查看 | 只读展示固定值 `SINGLE` |
| 5 | 券Code分配模式 | 查看 | 只读展示固定值 `UPLOAD` |
...
- `商品券ID`、`商品券类型`、`外部商品ID`、`使用模式`、`券Code分配模式` 均为只读基础信息，不允许修改；其中 `外部商品ID` 在活动未产生批次时可编辑，产生 1 个及以上批次后在任何页面均只读。
...
| TC-012 | MODIFY-01 | 正常流程 | 生效中活动进行低风险修改 | 活动状态为 `生效中` | 修改允许编辑的展示类字段并保存 | 生效中活动 | 保存成功，仅影响后续新发券 |
| TC-013 | MODIFY-02 | 规则流程 | 高风险字段保持只读 | 活动状态为 `生效中` | 尝试修改高风险字段 | 生效中活动 | 高风险字段不可编辑或不可进入 |
...
| 活动编辑 | [Weixin摇一摇_活动编辑页_v1.8_20260510.md](E:/Codex/New_rules/docs/versions/Weixin摇一摇/Weixin摇一摇_活动编辑页_v1.8_20260510.md) | 草稿 | 继续补全、保存、提交 | 审批中不可进入编辑页 | 是 |
