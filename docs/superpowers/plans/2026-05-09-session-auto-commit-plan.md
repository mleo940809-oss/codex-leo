...
**Architecture:** 采用“会话 checkpoint + 正式产物包 + Git 提交”三段式流程。会话中的讨论只在达到正式产物门槛时进入提交候选状态；候选状态必须先满足会话记忆沉淀、正式修订判定、快照、版本号和文件完整性校验，再在现有《文档仓库提交规范》约束下执行 `git add`、`git commit`，正式发布时补打标签。该方案不以“对话结束”为提交触发点，而以“阶段性产物完成”为触发点。

**Tech Stack:** Git、PowerShell、Markdown、现有文档规范体系、按文档分类的 `docs/versions/<文档名>/` 快照目录。
...
本计划只在现有工作分支内执行自动提交，不负责替代分支创建，也不重新定义仓库级快照与提交顺序。《文档仓库提交规范》为仓库级顺序权威；本计划只补会话门禁与自动提交触发条件。若当前会话还没有工作分支，应先创建或切换到合适的工作分支，再进入自动提交流程。
...
自动提交只定义门禁条件，不在本计划里重复定义仓库级执行顺序。进入候选前必须满足：

- 如果属于跨会话延续、长任务或压缩风险场景，先写会话记录并更新总索引
- 已确认本次修改属于正式修订
- 已确认当前处于合适的工作分支
- 已达到正式产物门槛
- 其余动作严格按《文档仓库提交规范》执行，不在本计划中重述
...
会话记忆沉淀 -> 正式修订判定 -> 工作分支确认 -> 正式产物确认 -> 快照生成 -> Git 提交 -> 正式发布/标签 -> 历史留存
